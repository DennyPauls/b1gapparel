...
**Live site:** https://www.lettermenrack.com (Vercel primary host; the apex lettermenrack.com 308-redirects to www, so all canonicals and sitemap URLs use www. DNS managed in GoDaddy)
...
`/schools/[school]` and `/schools/[school]/[sport]` are static HTML generated by `node tools/generate-school-pages.js` (no build step on Vercel; the generated files are committed). All page copy, titles and meta descriptions live in that script and it validates title/meta length and uniqueness. To add a school: add an entry to `SCHOOLS` (with hand-written `hubIntro` and per-sport `hooks`), re-run it, and add `hub:"<slug>"` to that school in `index.html` plus a link in the homepage `.team-links` list.
